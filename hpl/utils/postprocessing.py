...
def load_test_simulations(path: str, n_simulations: int, device: str = "cpu") -> torch.Tensor:
    """Load test L96 simulations from a file generated by script from mdml_tools.
...
def load_hydra_config(directory: str) -> DictConfig:
    ...
    try:
        ...
        raise FileNotFoundError(f"Could not find hydra config in {directory}.")
    return config
...
def find_experiments_directories(directories: list[str]) -> list[str]:
    ...
    This function searches for directories containing .hydra subdirectory.
    However, it does not check if the experiment was finished successfully.
